import { FaInstagram, FaEnvelope, FaPhone } from "react-icons/fa";

export function Footer() {
    return (
        <footer className="bg-[#0F172A] text-slate-300 border-t border-slate-800 mt-auto">

            <div className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-1 md:grid-cols-3 gap-8">

                {/* ABOUT */}
                <div>
                    <h2 className="text-white font-semibold text-lg mb-3">
                        Aticurando
                    </h2>
                    <p className="text-sm text-slate-400">
                        Cultura, educação e transformação social em Atibaia.
                        Conectando pessoas através de projetos educacionais e voluntariado.
                    </p>
                </div>

                {/* CONTACT */}
                <div>
                    <h2 className="text-white font-semibold text-lg mb-3">
                        Contato
                    </h2>

                    <div className="space-y-2 text-sm">
                        <p className="flex items-center gap-2">
                            <FaPhone /> +55 11 96286-5972
                        </p>

                        <p className="flex items-center gap-2">
                            <FaEnvelope /> contato@aticurando.org
                        </p>

                        <a
                            href="https://instagram.com/aticurando"
                            target="_blank"
                            className="flex items-center gap-2 hover:text-white transition"
                        >
                            <FaInstagram /> @aticurando
                        </a>
                    </div>
                </div>

                {/* LINKS / INDICAÇÕES */}
                <div>
                    <h2 className="text-white font-semibold text-lg mb-3">
                        Links úteis
                    </h2>

                    <ul className="space-y-2 text-sm">
                        <li>
                            <a href="/sobre" className="hover:text-white transition">
                                Sobre nós
                            </a>
                        </li>

                        <li>
                            <a href="/cursos" className="hover:text-white transition">
                                Cursos
                            </a>
                        </li>

                        <li>
                            <a href="/voluntariado" className="hover:text-white transition">
                                Seja voluntário
                            </a>
                        </li>
                    </ul>
                </div>

            </div>
            <div className="max-w-2xl mx-auto px-3 pb-3">
                <a
                    href="https://wa.me/5511962865972?text=Olá!%20Preciso%20de%20suporte"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="
                        flex items-center justify-center gap-2
                        rounded-xl bg-green-500 px-4 py-3
                        text-sm font-semibold text-white
                        transition
                        hover:bg-green-600 hover:shadow-lg
                        active:scale-95
                    "
                >
                     Falar com suporte no WhatsApp
                </a>
            </div>

            {/* BOTTOM BAR */}
            <div className="border-t border-slate-800 py-4 text-center text-xs text-slate-500">
                © {new Date().getFullYear()} Aticurando. Todos os direitos reservados.
            </div>
             


        </footer>
    );
}